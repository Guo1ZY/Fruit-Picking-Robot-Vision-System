 ...
 * @param Z
 */
void Uart_Thread_Space::Chassis_Infos_Send(Uart_Thread *uart_ptr, uint8_t left_x, uint8_t left_y, uint8_t right_x, uint8_t right_y)
{
    printf("CHassis Info Send!\n");

    /*为写串口缓冲区赋值*/
    uart_ptr->writeBuff[2] = 0x01;
    memcpy(&uart_ptr->writeBuff[3], &left_x, 1);
    memcpy(&uart_ptr->writeBuff[4], &left_y, 1);
    memcpy(&uart_ptr->writeBuff[5], &right_x, 1);
    memcpy(&uart_ptr->writeBuff[6], &right_y, 1);
}
