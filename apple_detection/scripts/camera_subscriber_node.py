...
"""
@file simplified_camera_subscriber_node.py
@author Guo1ZY
@brief Simplified ROS node for apple detection with 3D coordinate publishing
@version 0.1
@date 2025-06-28
"""
...
class AppleDetectionNode:
    def __init__(self):
        ...
        rospy.loginfo("Apple detection node initialized!")
    ...
    def get_3d_coordinate(self, u, v):
        ...
        try:
            ...
            if self.current_depth_image.dtype == np.uint16:
                depth_value = self.current_depth_image[v, u] / 1000.0  # 毫米转米
            elif self.current_depth_image.dtype == np.float32:
                depth_value = self.current_depth_image[v, u]
            else:
                return None
        ...
        try:
            ...
            for i, detection in enumerate(detections):
                ...
                if coord_3d is not None:
                    ...
                    detection_coordinates.append(detection_data)
            ...
            if detection_coordinates:
                self.publish_detection_results(detection_coordinates)
    ...
    def publish_detection_results(self, detection_coordinates):
        """发布检测结果"""
        try:
            # 创建Float32MultiArray消息
            msg = Float32MultiArray()
            
            # 数据格式：[num_detections, detection1_data, detection2_data, ...]
            # 每个detection_data: [class_id, confidence, center_x_2d, center_y_2d, x_3d, y_3d, z_3d]
            
            data = [len(detection_coordinates)]  # 检测数量
            
            for detection in detection_coordinates:
                # 添加检测数据
                data.extend([
                    float(detection['detection_id']),  # 检测ID
                    detection['confidence'],           # 置信度
                    float(detection['center_2d'][0]),  # 2D中心点x
                    float(detection['center_2d'][1]),  # 2D中心点y
                    detection['center_3d'][0],         # 3D坐标x
                    detection['center_3d'][1],         # 3D坐标y
                    detection['center_3d'][2]          # 3D坐标z
                ])
            
            msg.data = data
            self.coordinate_pub.publish(msg)
            
            rospy.logdebug(f"Published {len(detection_coordinates)} detection results")
            
        except Exception as e:
            rospy.logerr(f"Error publishing detection results: {str(e)}")
...
def main():
    ...
    try:
        ...
        rospy.loginfo("Apple detection node started!")
