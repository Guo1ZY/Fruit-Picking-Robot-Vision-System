 ...
 * @param Z
 */
void Uart_Thread_Space::Lidar_Pose_Send(Uart_Thread *uart_ptr, float X, float Y,float Z )
{
    printf("Lidar pose send!\n");

    /*为写串口缓冲区赋值*/
    uart_ptr->writeBuff[2] = 0x01;
    memcpy(&uart_ptr->writeBuff[3], &X, 4);
    memcpy(&uart_ptr->writeBuff[7], &Y, 4);
    memcpy(&uart_ptr->writeBuff[11], &Z, 4);
}

/**
 * @brief roll pitch yaw 发送
 * 
 * @param uart_ptr 
 * @param roll 
 * @param pitch 
 * @param yaw 
 */
void Uart_Thread_Space::Lidar_IMU_Send(Uart_Thread *uart_ptr, float roll, float pitch, float yaw)
{
    printf("IMU send!\n");

    /*为写串口缓冲区赋值*/
    uart_ptr->writeBuff[2] = 0x02;
    memcpy(&uart_ptr->writeBuff[3], &roll, 4);
    memcpy(&uart_ptr->writeBuff[7], &pitch, 4);
